...
class Down(nn.Module):
    ...
    def __init__(self, configs,num, in_channels, out_channels):
        ...
        self.down_sample=nn.Sequential(nn.Linear(4,8),nn.BatchNorm1d(in_channels),nn.ReLU(inplace=True)
                                       ,nn.Linear(8,4),nn.BatchNorm1d(in_channels),nn.ReLU(inplace=True)
                                       ,nn.Linear(4,1))
...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, linear=True):
        ...
        self.up1=nn.Sequential(nn.Linear(1,4),nn.BatchNorm1d(in_channels), nn.ReLU(inplace=True)
                               ,nn.Linear(4,8),nn.BatchNorm1d(in_channels),nn.ReLU(inplace=True),
                               nn.Linear(8,4))
